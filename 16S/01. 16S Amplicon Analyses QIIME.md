...
Once activated "qiime2-2021.2" will show up before your username. 
Ex:  
```
(qiime2-2021.2) [egrant@zenith 16S]$
```
...
source /home/opt/Miniconda2/miniconda2/bin/activate qiime2-2021.2
...
```
B. Remove Chloroplast, Mitochondria, Unassigned and Eukaryota.
```
qiime taxa filter-table \
--i-table /home/projects/Agribiome/Kerbel/16S/03.DADA2/table.qza \
--i-taxonomy taxonomy_silva138_16S.qza \
--p-exclude Mitochondria,Chloroplast,Unassigned,Eukaryota \
--o-filtered-table taxonomy_silva138_16S_clean.qza
```
C. Visualize your CLEAN taxonomy table.
```
qiime metadata tabulate \
--m-input-file taxonomy_silva138_16S_clean.qza \
--o-visualization taxonomy_silva138_16S_clean.qzv
```
...
```
B. Visualize your taxonomy table.   
```
qiime metadata tabulate \
--m-input-file taxonomy_gtdb_220.qza \
--o-visualization taxonomy_gtdb_220.qzv
```
# 6. Download tables for downstream analysis  
This table has the ASV and the counts per sample. We will merge this "clean" table with the GTDB taxonomy in R via leftjoin to ensure that all mitochondria, chloroplast and Euk ASV's are actually removed. 
```
# This will creat a folder with long numbers ex: <6b5fd43f-240f-490a-85e7-26879af8d96c>
unzip table-clean.qza
```
Navigate into the numerical folder and then into the "data" directory. You can convert the .biom table to a .txt file if needed. Many downstream analysis tools can use the .biom file format. 
```
biom convert -i feature-table.biom -o table.from_biom.txt --to-tsv
```
Note: Rarification can be done in QIIME however, for this data set we rarify in R. 
